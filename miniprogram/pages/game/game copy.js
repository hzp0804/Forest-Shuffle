...
Page({
  ...
  onLoad(options) {
    ...
    this.setData = (data, callback) => {
      if (data.myHand) {
        console.log("myHand changed:", data.myHand);
      }
  ...
  handleGameUpdate(room) {
    ...
    const findSpeciesMeta = (code = "") => {
      ...
      // Fallbacks: case-insensitive and stripped spacing/punctuation (e.g., "Douglas Fir" -> "DouglasFir")
      const target = normalize(code);
      const key = Object.keys(SPECIES_DATA).find(
        (k) => normalize(k) === target || k.toLowerCase() === code.toLowerCase()
      );
    ...
    // Robust check for turn；单人模式直接视为自己的回合
    let isMyTurn = turnOrder.length <= 1 || turnOrder[currentIdx] === myOpenId;
  ...
  onCardTap(e) {
    ...
    // 打印选中的手牌
    const selectedCards = newHand.filter((c) => c.selected);
    console.log("选中手牌", selectedCards);
  },
  ...
  async onDrawCard() {
    ...
    try {
      ...
      if (
        ...
        latestOrder[gameState.currentPlayerIdx || 0] !== myId
      ) {
        wx.showToast({ title: "当前不是你的回合", icon: "none" });
        return;
      }
