...
async function processNextEvent(page) {
  ...
  if (page.data.eventQueue.length === 0) {
    ...
    if (page.data.pendingActionToast) {
      wx.showToast({ title: page.data.pendingActionToast, icon: "none", duration: 1500 });
  ...
  page.setData({ currentEvent: event, eventQueue: remaining, isCardFlipped: false });

  // === 自动切换视角逻辑 ===
  // 只有在回合切换时才自动跟随视角，其他玩家行动（打牌、抽牌等）不再强制拉走用户视角
  const autoSwitchTypes = ['TURN_CHANGE'];
  const targetOpid = event.playerOpenId;
  if (autoSwitchTypes.includes(event.type) && targetOpid && targetOpid !== page.data.selectedPlayerOpenId) {
    ...
    if (viewingPlayerState) {
      ...
      const viewingPlayer = page.data.players.find(p => p && p.openId === targetOpid);

      const syncedSlot = viewingPlayerState.selectedSlot || null;
      const targetIndex = page.data.players.findIndex(p => p && p.openId === targetOpid);
  ...
  if (needsFlip) {
    setTimeout(() => { page.setData({ isCardFlipped: true }); }, 50);
  } else {
