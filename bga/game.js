...
define([
  ...
  return declare("customgame.game", ebg.core.gamegui, {
    ...
    onEnteringState(stateName, args) {
      ...
      this.overcost = args.args && args.args.overcost ? args.args.overcost : 0;
    ...
    onClick(node, callback, temporary = true) {
      ...
      if (!nodeElem) {
        debug("Unknow Element in onClick function", node, callback, temporary);
        return;
      }
      ...
      if (temporary) {
        this.connect(nodeElem, "click", safeCallback);
        dojo.removeClass(node, "unselectable");
        dojo.addClass(node, "selectable");
        this._selectableNodes.push(node);
      } else {
        dojo.connect(nodeElem, "click", safeCallback);
      }
    },
    ...
    addCustomTooltip(id, html, delay) {
      ...
      dojo.connect($(id + "-help"), "click", (evt) => {
        if (!this._helpMode) {
          tooltip.close();
        } else {
          evt.stopPropagation();
